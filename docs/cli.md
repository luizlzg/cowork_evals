...
- **A verb names its object only when that object is not an eval.** `run` runs evals, which
  ...
  `setup`, `login`, `check` and `prune` act on a backend and carry no object at all. `docs`
  and `init` act on neither: they read what the package ships and write into the working
  directory.
